#!/bin/bash
# Context bar statusline for Claude Code
    ...
    if [[ -n "$branch" ]]; then
        ...
        if [[ -n "$upstream" ]]; then
            ...
            if [[ "$ahead" -eq 0 && "$behind" -eq 0 ]]; then
                sync_status="synced${fetch_ago:+ ${fetch_ago}}"
        ...
        if [[ "$file_count" -eq 0 ]]; then
            git_status="(0 files uncommitted${sync_status:+, ${sync_status}})"
        elif [[ "$file_count" -eq 1 ]]; then
            single_file=$(git -C "$cwd" --no-optional-locks status --porcelain -uall 2>/dev/null | head -1 | sed 's/^...//')
            git_status="(${single_file} uncommitted${sync_status:+, ${sync_status}})"
        else
            git_status="(${file_count} files uncommitted${sync_status:+, ${sync_status}})"
        fi
    fi
fi
